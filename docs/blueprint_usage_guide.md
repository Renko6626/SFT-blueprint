...
uv sync --quiet
uv run accelerate launch ... train_sft.py ...
```
...
如果启用了 `test_mode`，这个字段在运行时不会被实际使用，但保留它可以让 blueprint 表单契约保持稳定。
...
- 在 `test_mode` 下，也会作为 `--test_model_name_or_path` 传给训练脚本
...
是否启用内建云端 smoke workflow。

启用后：

- blueprint 会向训练脚本传入 `--test_mode`
- 训练脚本会自动从 Hugging Face 下载一小份测试数据
- 会优先用于验证 Magnus 环境、依赖安装、网络访问和训练入口是否完整可用
- 更适合作为新仓库接入 Magnus 时的第一轮测试
...
- `output_root` 是否可写
- `CONDA_SH` 和 `CONDA_ENV` 是否与 Magnus 实际环境一致
- 在仓库根目录执行 `uv sync --quiet` 是否能成功
